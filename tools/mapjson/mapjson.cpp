    ...
    if (map_data["object_events"].array_items().size() > 0) {
        ...
        for (unsigned int i = 0; i < map_data["object_events"].array_items().size(); i++) {
            ...
            if (type == "" || type == "object") {
                ...
                // quest_id and menacing_trainer share the one icon_param halfword
                // at most one should be set
                string icon_param = json_to_string(obj_event, "quest_id", true);
                if (icon_param.empty())
                    icon_param = json_to_string(obj_event, "menacing_trainer", true);
                text << ", " << (icon_param.empty() ? "QUEST_NONE" : icon_param) << "\n";
            } else if (type == "clone") {
