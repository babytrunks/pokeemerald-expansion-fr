...
#define OW_FOLLOWER_MEGA_FORMS         TRUE       // If TRUE, a follower holding a Mega Stone (or knowing the Mega trigger move, e.g. Rayquaza) displays its Mega form in the overworld. Requires OW_BATTLE_ONLY_FORMS.
...
#define OW_FLAG_NO_COLLISION        FLAG_SYS_COLLISION_OFF  // If this flag is set, the player will be able to walk over tiles with collision. Mainly intended for debugging purposes.
#define OW_FLAG_POKE_RIDER          FLAG_SYS_POKEDEX_GET  // If this flag is set, the player will be able to use fly from the Pokenav Region Map and the Town Map key item by pressing 'R' on a city/location they are able to fly to.
