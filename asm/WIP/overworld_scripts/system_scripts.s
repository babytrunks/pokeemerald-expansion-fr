...
.equ FLAG_BIKE_TURBO_BOOST, 0x91F
@;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
SystemScript_EnableAutoRun:
	...
	@ if 0x0 _goto NoPokeVial
	msgboxsign
	lockall
	setvar 0x8006 0x0 @first item
	loadpointer 0x0 gText_AutoRun
	special 0x25
	checkflag FLAG_INFINITE_REPEL_UNLOCK
	if SET _call SetRepelString
	checkflag FLAG_INFINITE_REPEL_UNLOCK
	if NOT_SET _call SetUnknownSkill2
	checkflag FLAG_ACTIVATE_POKEVIAL
	if SET _call SetPokeVialString
	checkflag FLAG_ACTIVATE_POKEVIAL
	if NOT_SET _call SetUnknownSkill3
	checkflag FLAG_UNLOCK_TIME_CHANGER
	if SET _call SetTimeChangerString
	checkflag FLAG_UNLOCK_TIME_CHANGER
	if NOT_SET _call SetUnknownSkill4
    preparemsg gText_ChooseWhichAddon
    waitmsg
    multichoice 0x0 0x0 0x22 0x0
    compare LASTRESULT 0x0
    if 0x1 _goto AutoRun
    compare LASTRESULT 0x1
    if 0x1 _goto InfRepel
    compare LASTRESULT 0x2
    if 0x1 _goto PokeVial
	compare LASTRESULT 0x3
	if 0x1 _goto TimeChanger
	release
	end

SetRepelString:
	setvar 0x8006 0x1 @second item
	loadpointer 0x0 gText_InfRepel
	special 0x25
	return

SetUnknownSkill2:
	setvar 0x8006 0x1
	loadpointer 0x0 gText_UnknownSkill
	special 0x25
	return

SetPokeVialString:
	setvar 0x8006 0x2 @third item
    loadpointer 0x0 gText_PokeVial
    special 0x25
	return

SetTimeChangerString:
	setvar 0x8006 0x3 @fourth item
	loadpointer 0x0 gText_TimeChanger
	special 0x25
	return
...
TimeChanger:
	checkflag FLAG_UNLOCK_TIME_CHANGER
	if NOT_SET _goto SkillIsntUnlocked
	msgboxsign
	lockall
	setvar 0x8006 0x0 @first item
	loadpointer 0x0 gText_Day
	special 0x25
	setvar 0x8006 0x1 @second item
	loadpointer 0x0 gText_Night
	special 0x25
	setvar 0x8006 0x2 @third item
    loadpointer 0x0 gText_Dusk
    special 0x25
    setvar 0x8006 0x3 @fourth item
    loadpointer 0x0 gText_Reset
    special 0x25
	preparemsg gText_ChooseWhichTime
    waitmsg
	multichoice 0x0 0x0 0x22 0x0
	compare LASTRESULT 0x0
	if equal _goto SetDay
	compare LASTRESULT 0x1
	if equal _goto SetNight
	compare LASTRESULT 0x2
	if equal _goto SetDusk
	compare LASTRESULT 0x3
	if equal _goto ResetTime
	goto SystemScript_EnableAutoRun
	end
...
AutoRun: 
	msgboxsign
	checkflag 0x914
	if 0x1 _goto SystemScript_DisableAutoRun
	checksound
	sound 0x2
	msgbox gText_AutoRunEnable MSG_NORMAL
	setflag 0x914 
	releaseall
	end
...
InfRepel:
	...
	if NOT_SET _goto SkillIsntUnlocked
	checkflag 0x1045
	if 0x0 _goto EnableInfRepel
	checksound
	sound 0x3
	msgbox gText_AutoRepelDisable MSG_NORMAL
	clearflag 0x1045
	release
	end
...
SkillIsntUnlocked:
	msgboxsign
	msgbox gText_SkillIsntUnlocked MSG_NORMAL
	goto SystemScript_EnableAutoRun
	end
...
EnableInfRepel:
	...
	msgbox gText_AutoRepelEnable MSG_NORMAL
	setflag 0x1045
	releaseall
	end
...
PokeVial:
	...
	if NOT_SET _goto SkillIsntUnlocked
	compare 0x5129 0x0
	if equal _goto OutOfDoses
	buffernumber 0x0 0x5129
	msgbox gText_UsePokeVial MSG_YESNO
	compare LASTRESULT YES
	if equal _goto UsePokeVial
	release
	end

.equ VAR_TEMPORARY_FOR_FOLLOW, 0x5140
...
UsePokeVial:
	...
	subvar 0x5129 0x1
	buffernumber 0x0 0x5129
	msgbox gText_PokeVialUsed MSG_NORMAL
	checkflag FLAG_FOLLOWER_IN_PROGRESS
	if 0x1 _call UpdatePokemonFollowerThing
	release
	end

UpdatePokemonFollowerThing: 
	@ fadescreen 0x1
	copyvar VAR_TEMPORARY_FOR_FOLLOW 0x5130
...
SoundBall:
	...
	return
OutOfDoses:
	msgbox gText_NoPokeVialsAvailable MSG_NORMAL
	release
	end
...
SystemScript_DisableAutoRun:
	...
	clearflag 0x914 
	msgboxsign
	msgbox gText_AutoRunDisable MSG_NORMAL
	releaseall
	end
...
SystemScript_EnableBikeTurboBoost:
	...
	msgboxsign
	setvar 0x8006 0x0 @first item
	loadpointer 0x0 gText_TurboBoost
	special 0x25
	checkflag FLAG_INFINITE_REPEL_UNLOCK
	if SET _call SetRepelString
	checkflag FLAG_INFINITE_REPEL_UNLOCK
...
TurboBoost:
	checkflag FLAG_BIKE_TURBO_BOOST
	if 0x1 _goto DisableTurboBoost
	lockall
...
DisableTurboBoost:
	...
	checksound
	clearflag FLAG_BIKE_TURBO_BOOST
	releaseall
	end
...
EventScript_UseWaterfall:
	...
	@ checkflag FLAG_AUTO_HMS
	@ if SET _
	goto EventScript_UseWaterfall_SkipAsk
...
.global EventScript_UseFlash2
EventScript_UseFlash2:
	lockall
	setflag FLAG_SYS_USE_FLASH
	checksound
	sound 0xC8
