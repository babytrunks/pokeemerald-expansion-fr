    ...
    while (remainingWidth > 0)
    {
        ...
        else
        {
            ...
            for (u32 row = 0; row < height; row++)
            {
                ...
                u32 newPixels;
                if (isColor)
                    newPixels = color & srcMask;
                else
                    newPixels = src[CURRENT_SPRITE_POS] & srcMask;

                tiles[CURRENT_SPRITE_POS] = orig | newPixels;
                if (row == height - 1)
                {
                    currSpriteId = spriteId;
                    tiles = (u32 *)((OBJ_VRAM0) + gSprites[currSpriteId].oam.tileNum * TILE_SIZE_4BPP);
...
// Links a grid of equally sized sprites so text can flow across them
