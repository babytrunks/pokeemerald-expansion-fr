...
enum WeatherFailure TryChangeBattleWeather(u32 battler, u32 battleWeatherId, u32 ability)
{
    if (gBattleWeather & sBattleWeatherInfo[battleWeatherId].flag)
    {
        return WEATHER_FAILURE_SAME_WEATHER;
    }
    else if (gBattleStruct->overworldWeatherPresent && gBattleWeather & B_WEATHER_ANY)
    {
...
// Handles the ability pop-up and message for weather setting abilities, whether they succeed or are blocked.
static bool32 TryChangeWeatherWithAbility(u32 battler, u32 battleWeatherId, u32 ability, const u8 *successScript)
{
    switch (TryChangeBattleWeather(battler, battleWeatherId, ability))
    {
    case WEATHER_FAILURE_SUCCESS:
        BattleScriptCall(successScript);
        return TRUE;
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, u32 battler, enum Ability ability, u32 move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_DRIZZLE:
            if (shouldAbilityTrigger && TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_RAIN, gLastUsedAbility, BattleScript_DrizzleActivates))
                effect++;
            break;
        case ABILITY_SAND_STREAM:
            if (shouldAbilityTrigger && TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_SANDSTORM, gLastUsedAbility, BattleScript_SandstreamActivates))
                effect++;
            break;
        case ABILITY_ORICHALCUM_PULSE:
        case ABILITY_DROUGHT:
            if (shouldAbilityTrigger && TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_SUN, gLastUsedAbility, BattleScript_DroughtActivates))
                effect++;
            break;
        case ABILITY_SNOW_WARNING:
            if (!shouldAbilityTrigger)
                break;
            if (GetGenConfig(GEN_SNOW_WARNING) >= GEN_9)
            {
                if (TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_SNOW, gLastUsedAbility, BattleScript_SnowWarningActivatesSnow))
                    effect++;
            }
            else
            {
                if (TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_HAIL, gLastUsedAbility, BattleScript_SnowWarningActivatesHail))
                    effect++;
            }
            break;
        case ABILITY_ELECTRIC_SURGE:
        case ABILITY_HADRON_ENGINE:
            if (shouldAbilityTrigger && TryChangeBattleTerrain(battler, STATUS_FIELD_ELECTRIC_TERRAIN))
            {
        ...
        case ABILITY_GRASSY_SURGE:
            if (shouldAbilityTrigger && TryChangeBattleTerrain(battler, STATUS_FIELD_GRASSY_TERRAIN))
            {
        ...
        case ABILITY_MISTY_SURGE:
            if (shouldAbilityTrigger && TryChangeBattleTerrain(battler, STATUS_FIELD_MISTY_TERRAIN))
            {
        ...
        case ABILITY_PSYCHIC_SURGE:
            if (shouldAbilityTrigger && TryChangeBattleTerrain(battler, STATUS_FIELD_PSYCHIC_TERRAIN))
            {
        ...
        case ABILITY_DESOLATE_LAND:
            if (shouldAbilityTrigger && TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_SUN_PRIMAL, gLastUsedAbility, BattleScript_DesolateLandActivates))
                effect++;
            break;
        case ABILITY_PRIMORDIAL_SEA:
            if (shouldAbilityTrigger && TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_RAIN_PRIMAL, gLastUsedAbility, BattleScript_PrimordialSeaActivates))
                effect++;
            break;
        case ABILITY_DELTA_STREAM:
            if (shouldAbilityTrigger && TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_STRONG_WINDS, gLastUsedAbility, BattleScript_DeltaStreamActivates))
                effect++;
            break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_SAND_SPIT:
             ...
             && !(gBattleWeather & B_WEATHER_SANDSTORM && HasWeatherEffect()))
            {
                ...
                if (TryChangeWeatherWithAbility(battler, BATTLE_WEATHER_SANDSTORM, gLastUsedAbility, BattleScript_SandSpitActivates))
                    effect++;
            }
            break;
...
bool32 IsBattlerProtected(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (IsSideProtected(battlerDef, PROTECT_CRAFTY_SHIELD)
     ...
     && GetMoveEffect(move) != EFFECT_COACHING)
        ...
        isProtected = FALSE;
    else if (IsSideProtected(battlerDef, PROTECT_WIDE_GUARD)
          && (moveTarget == MOVE_TARGET_BOTH || moveTarget == MOVE_TARGET_FOES_AND_ALLY)) // Wide Guard also works in Single Battles
        isProtected = TRUE;
...
{
    if (ctx->abilityAtk == ABILITY_MEGA_SOL && ctx->moveType == TYPE_FIRE)
        return UQ_4_12(1.5);
...
void UpdateMoveResultFlags(uq4_12_t modifier, u16 *resultFlags)
{
    if (modifier == UQ_4_12(0.0))
    {
        ...
        *resultFlags &= ~(MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE);
        gBattleStruct->blunderPolicy = FALSE; // Don't activate if missed
    }
    else if (modifier == UQ_4_12(1.0))
    {
        *resultFlags &= ~(MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_DOESNT_AFFECT_FOE | MOVE_RESULT_EXTREMELY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE);
    }
    else if (modifier >= UQ_4_12(4.0))
    {
        *resultFlags |= MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_EXTREMELY_EFFECTIVE;
        *resultFlags &= ~(MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_DOESNT_AFFECT_FOE | MOVE_RESULT_MOSTLY_INEFFECTIVE);
    }
    else if (modifier > UQ_4_12(1.0))
    {
        *resultFlags |= MOVE_RESULT_SUPER_EFFECTIVE;
        *resultFlags &= ~(MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_DOESNT_AFFECT_FOE | MOVE_RESULT_EXTREMELY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE);
    }
    else if (modifier <= UQ_4_12(0.25))
    {
        *resultFlags |= MOVE_RESULT_NOT_VERY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE;
        *resultFlags &= ~(MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_DOESNT_AFFECT_FOE | MOVE_RESULT_EXTREMELY_EFFECTIVE);
    }
    else //if (modifier < UQ_4_12(1.0))
    {
        *resultFlags |= MOVE_RESULT_NOT_VERY_EFFECTIVE;
        *resultFlags &= ~(MOVE_RESULT_SUPER_EFFECTIVE | MOVE_RESULT_DOESNT_AFFECT_FOE | MOVE_RESULT_EXTREMELY_EFFECTIVE | MOVE_RESULT_MOSTLY_INEFFECTIVE);
    }
}
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    else if (ctx->moveType == TYPE_ROCK && ctx->abilityDef == ABILITY_MOUNTAINEER)
    {
        modifier = UQ_4_12(0.0);
        if (ctx->updateFlags)
        {
...
void TryRestoreHeldItems(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9 || gBattleStruct->itemLost[B_SIDE_PLAYER][i].stolen || returnNPCItems)
        {
            ...
            // Restore the original held item (including berries) consumed/lost in battle
...
bool32 IsBattlerAffectedByHazards(u32 battler, enum HoldEffect holdEffect, bool32 toxicSpikes)
{
    ...
    else if (holdEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS)
    {
        ...
        || GetBattlerAbility(battler) == ABILITY_FROZEN_MIST)
    {
        ret = FALSE;
    }
    return ret;
...
{
    // Mega Sol: treats all sun-dependent checks as if sun is active
...
{
    if (ability == ABILITY_SHIELD_DUST || ability == ABILITY_FROZEN_MIST)
    {
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    else if (move == MOVE_GUNK_SHOT && IS_BATTLER_OF_TYPE(battlerAtk, TYPE_POISON))
    {
        effect = TRUE;
    }
