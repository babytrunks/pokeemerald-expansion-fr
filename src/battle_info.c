...
// Base total durations for displayed "remaining/total" counters.
// This codebase has no B_*_TIMER defines; values match the assignment sites
// in battle_script_commands.c / battle_util.c.
...
static void DetailRefreshTeraTypeIndicator(void)
{
    ...
    // Unconfigured (0) aliases FLAG_TEMP_0 in this FRLG-based project, so bail out explicitly.
...
static void DetailBuildActiveEffectsForBattler(void)
{
    ...
    struct DisableStruct *disable = &gDisableStructs[battler];
    ...
    TryAddActiveSideStatus(INFO_TAILWIND, SIDE_STATUS_TAILWIND, sideStatus->tailwindTimer, (B_TAILWIND_TURNS >= GEN_5 ? 4 : 3), side);
    ...
    TryAddActiveStatus(INFO_AUTOTOMIZE, disable->autotomizeCount, side);
    TryAddActiveStatus(INFO_TAR_SHOT, disable->tarShot, side);
    TryAddActiveStatus(INFO_OCTOLOCK, disable->octolock, side);
    ...
    TryAddActiveStatus(INFO_TAUNT, disable->tauntTimer, side);
    TryAddActiveStatus(INFO_CHARGE, vol->chargeTimer, side);
    TryAddActiveStatus(INFO_BOUND, disable->wrapTurns, side);
    TryAddActiveStatus(INFO_RAMPAGING, vol->lockConfusionTurns, side);
    TryAddActiveStatus(INFO_CONFUSION, vol->confusionTurns, side);
    ...
    TryAddActiveStatusTimer(INFO_HEALING_PREVENTED, disable->healBlockTimer, B_HEAL_BLOCK_TIMER, side);
    TryAddActiveStatusTimer(INFO_EMBARGO, disable->embargoTimer, B_EMBARGO_TIMER, side);
    TryAddActiveStatusTimer(INFO_TELEKINESIS, disable->telekinesisTimer, B_TELEKINESIS_TIMER, side);
    TryAddActiveStatusTimer(INFO_MAGNET_RISE, disable->magnetRiseTimer, B_MAGNET_RISE_TIMER, side);
    TryAddActiveStatusTimer(INFO_BIDE, vol->bideTurns, 2, side);
    TryAddActiveStatusTimer(INFO_FUTURE_ATTACK, gWishFutureKnock.futureSightCounter[battler], 3, side);
    TryAddActiveStatusTimer(INFO_THROAT_CHOP, disable->throatChopTimer, B_THROAT_CHOP_TIMER, side);
    TryAddActiveStatusTimer(INFO_LASER_FOCUS, disable->laserFocusTimer, B_LASER_FOCUS_TIMER, side);
    TryAddActiveStatusTimer(INFO_SLOW_START, disable->slowStartTimer, B_SLOW_START_TIMER, side);
    TryAddActiveStatusTimer(INFO_SYRUPY, disable->syrupBombTimer, B_SYRUP_BOMB_TIMER, side);
    TryAddActiveStatusTimer(INFO_WISH, gWishFutureKnock.wishCounter[battler], 2, side);

    if (B_DISABLE_TURNS >= GEN_5)
        TryAddActiveStatusTimer(INFO_MOVE_DISABLED, disable->disableTimer, B_DISABLE_TIMER, side);
    else
        TryAddActiveStatus(INFO_MOVE_DISABLED, disable->disableTimer, side);

    if (B_UPROAR_TURNS >= GEN_5)
        TryAddActiveStatusTimer(INFO_UPROAR, vol->uproarTurns, B_UPROAR_TIMER, side);
    else
        TryAddActiveStatus(INFO_UPROAR, vol->uproarTurns, side);

    // Encore always lasts 3 turns in this codebase (Gen5+ behavior).
    TryAddActiveStatusTimer(INFO_ENCORE, disable->encoreTimer, B_ENCORE_TIMER, side);
}
...
static void TryAddActiveWeather(enum BattleInfoLabels label, enum BattleSide side)
{
    if (gWishFutureKnock.weatherDuration > 0)
    {
        struct DisplayTimer timer = {
            ...
            .remaining = gWishFutureKnock.weatherDuration,
        };
...
static enum BattleInfoLabels GetStatusEffectFromWeather(void)
{
    ...
    switch (GetCurrentBattleWeather()) // Returns 0xFF when no weather is set.
    {
...
static enum BattleInfoLabels GetStatusEffectFromTerrain(void)
{
    if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        return INFO_ELECTRIC_TERRAIN;
    if (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN)
        return INFO_GRASSY_TERRAIN;
    if (gFieldStatuses & STATUS_FIELD_MISTY_TERRAIN)
        return INFO_MISTY_TERRAIN;
    if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN)
        return INFO_PSYCHIC_TERRAIN;
...
static enum BattleInfoLabels GetStatusEffectFromNonVolatile(enum BattlerId battler)
{
    ...
    if (status1 & STATUS1_TOXIC_POISON)
        return INFO_BADLY_POISONED;
    if (status1 & STATUS1_POISON)
        return INFO_POISONED;
    if (status1 & STATUS1_PARALYSIS)
        return INFO_PARALYZED;
    if (status1 & STATUS1_BURN)
        return INFO_BURNED;
    if (status1 & STATUS1_FROSTBITE)
        return INFO_FROSTBITE;
...
static const u8 *GetPlayerSideTrainerName(void)
{
    static const u8 sText_Ally[] = _("Ally");
