...
enum BWBattleUICursorMode BattleUI_GetCursorMode(void)
{
    ...
    // SPRITE_NONE is 0xFF, well past the end of gSprites, so never index with it.
    if (spriteId == SPRITE_NONE)
        return BUI_CURSOR_MODE_HIDDEN;

    // filter out BUI_CURSOR_CONVERT_FLAG
    return (gSprites[spriteId].sCursorMode & 0xFF);
}

void BattleUI_DisplayMoveBox(u32 battler)
{
    CopyRectToBgTilemapBufferRect(0, sBWBattleUI_TextboxTilemapRaw,
        0, 0,
...
static void BattleUI_DisplayNormalMoveBox(u32 battler, struct ChooseMoveStruct *moveInfo)
{
    ...
    for (u32 i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // type
        u32 type = (moveId == MOVE_NONE) ? TYPE_MYSTERY : GetMoveType(moveId);
...
static void BattleUI_DisplayZMoveBox(u32 battler, struct ChooseMoveStruct *moveInfo)
{
    ...
    // type
    u32 type = (zMove == MOVE_NONE) ? TYPE_MYSTERY : GetMoveType(zMove);
