...
static void PrintMonStats(bool8 calcInitialEvTotal);
...
static const u8 sText_MonLives[]         = _("Lives: {STR_VAR_1}");
static const u8 sText_EVCap[]         = _("/{STR_VAR_1}");
...
static void PrintMonStats(bool8 calcInitialEvTotal)
{
    ...
    StringExpandPlaceholders(gStringVar2, sText_EVCap);
    AddTextPrinterParameterized4(WINDOW_2, 1, STARTING_X + THIRD_COLUMN_ISH, STARTING_Y + (DISTANCE_BETWEEN_STATS_Y * 6), 0, 0, sMenuWindowFontColors[FONT_WHITE], 0xFF, gStringVar2);
...
static void Task_StatEditorMain(u8 taskId) // input control when first loaded into menu
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        if((sStatEditorDataPtr->editingStat == 255 || (sStatEditorDataPtr->evTotal >= evCap)) && (sStatEditorDataPtr->selector_x == 0))
    ...
    switch(input)
    {
       ...
       case EDIT_INPUT_MAX_DECREASE_STATE:
        ...
        case EDIT_INPUT_MAX_INCREASE_STATE:
            ...
            {
                u32 evCap = getEvCap();
                if (evCap - sStatEditorDataPtr->evTotal < EV_MAX_SINGLE_STAT)
                    sStatEditorDataPtr->editingStat += evCap - sStatEditorDataPtr->evTotal;
                else
