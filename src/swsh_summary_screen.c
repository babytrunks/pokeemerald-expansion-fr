...
// Layout of the name bar, in PSS_LABEL_WINDOW_PORTRAIT_INFO pixels
// The gigantamax mark sits in the gap left of the name when it is enabled
#define PORTRAIT_NAME_X       (SWSH_SUMMARY_SHOW_GIGANTAMAX ? 22 : 4)
#define PORTRAIT_LEVEL_RIGHT  118 // level block is right aligned to this
#define PORTRAIT_NAME_RIGHT   90  // name limit when the status icon hides the level
...
static u8 SwSh_FormatTextByWidth(u8*, s32, u8, const u8*, s16);
static u8 SwSh_FormatTextByWidthLines(u8*, s32, u8, const u8*, s16, u32, u32*);
...
static const u8 sText_LevelCapSeparator[]       = _("/");
...
// bg gfx
static const u32 sSummaryScreen_Gfx[]                      = INCBIN_U32("graphics/summary_screen/swsh/tiles.4bpp.smol");
static const u16 sSummaryScreen_Pal[]                      = INCBIN_U16("graphics/summary_screen/swsh/tiles.gbapal");
static const u32 sSummaryPage_ScrollBG_Tilemap[]           = INCBIN_U32("graphics/summary_screen/swsh/scroll_bg.bin.smolTM");
static const u32 sSummaryPage_Info_Tilemap[]               = INCBIN_U32("graphics/summary_screen/swsh/page_info.bin.smolTM");
static const u32 sSummaryPage_Info_Item_Tilemap[]          = INCBIN_U32("graphics/summary_screen/swsh/page_info_item.bin.smolTM");
static const u32 sSummaryPage_Skills_Tilemap[]             = INCBIN_U32("graphics/summary_screen/swsh/page_skills.bin.smolTM");
static const u32 sSummaryPage_Skills_Dynamax_Tilemap[]     = INCBIN_U32("graphics/summary_screen/swsh/page_skills_dynamax.bin.smolTM");
static const u32 sSummaryPage_BattleMoves_Tilemap[]        = INCBIN_U32("graphics/summary_screen/swsh/page_battle_moves.bin.smolTM");
static const u32 sSummaryEffect_Battle_Tilemap[]           = INCBIN_U32("graphics/summary_screen/swsh/effect_battle.bin.smolTM");
static const u32 sSummaryPage_ContestMoves_Tilemap[]       = INCBIN_U32("graphics/summary_screen/swsh/page_contest_moves.bin.smolTM");
static const u32 sSummaryEffect_Contest_Tilemap[]          = INCBIN_U32("graphics/summary_screen/swsh/effect_contest.bin.smolTM");
#if SWSH_SUMMARY_SHOW_CONTEST_PAGES
static const u32 sSummaryPage_Conditions_Tilemap[]         = INCBIN_U32("graphics/summary_screen/swsh/page_conditions.bin.smolTM");
#endif
static const u32 sSummaryPage_Memo_Tilemap[]               = INCBIN_U32("graphics/summary_screen/swsh/page_memo.bin.smolTM");
...
static const u16 sDynamaxLevels_Pal[]               = INCBIN_U16("graphics/summary_screen/swsh/dynamax_levels.gbapal");
static const u16 sMoveSlot_Pals[]                   = INCBIN_U16("graphics/summary_screen/swsh/move_slot.gbapal");
static const u16 sMonShadow_Pal[]                   = INCBIN_U16("graphics/summary_screen/swsh/shadow.gbapal");
...
static const struct WindowTemplate sSummaryTemplate[] =
{
    ...
    // starts 2 tiles further left than the name bar art so the level cap fits
...
static const struct WindowTemplate sPageSkillsTemplate[] =
{
    ...
    [PSS_DATA_WINDOW_SKILLS_ABILITY] = {
        ...
        .width = 18,
        .height = SWSH_SUMMARY_SHOW_DYNAMAX_LEVEL ? 5 : 7,
        .paletteNum = 2,
        .baseBlock = 281,
    },
};
...
// FRLG only entry point so Deoxys shows the form it is currently in
// No flag is needed here, ExtractMonDataToSummaryStruct always reads the
// stored stats off currentMon, which BoxMonToMon has already recalculated
...
// This fork counts relearnable moves per struct Pokemon rather than per BoxPokemon
static bool32 HasAnyRelearnableMoves(enum MoveRelearnerStates state)
{
    struct Pokemon *mon = &sMonSummaryScreen->currentMon;

    if (!CheckRelearnerStateFlag(state) || GetMonData(mon, MON_DATA_IS_EGG))
        return FALSE;

    switch (state)
    {
    case MOVE_RELEARNER_LEVEL_UP_MOVES:
        return GetNumberOfLevelUpMoves(mon) != 0;
    case MOVE_RELEARNER_EGG_MOVES:
        return GetNumberOfEggMoves(mon) != 0;
    case MOVE_RELEARNER_TM_MOVES:
        return GetNumberOfTMMoves(mon) != 0;
    case MOVE_RELEARNER_TUTOR_MOVES:
        return GetNumberOfTutorMoves(mon) != 0;
    default:
        return FALSE;
    }
}
...
static void ChangeSummaryPokemon(u8 taskId, s8 delta)
{
    ...
    if (!sMonSummaryScreen->lockMonFlag)
    {
        ...
        // This fork has no full team multi battles, the partner uses gPlayerParty[3..5]
...
static void PrintNotEggInfo(void)
{
    ...
    u32 nameRight = PORTRAIT_NAME_RIGHT;
    u32 levelX = 0, levelLabelX = 0;

    // build the level string first, the name gets whatever room is left over
    if (statusAnim == 0)
    {
        u8 *end = ConvertIntToDecimalStringN(gStringVar2, summary->level, STR_CONV_MODE_LEFT_ALIGN, 3);

        if (SWSH_SUMMARY_SHOW_LEVEL_CAP)
        {
            end = StringCopy(end, sText_LevelCapSeparator);
            ConvertIntToDecimalStringN(end, GetCurrentLevelCap(), STR_CONV_MODE_LEFT_ALIGN, 3);
        }

        levelX = PORTRAIT_LEVEL_RIGHT - GetStringWidth(PSS_DEFAULT_FONT, gStringVar2, 0);
        levelLabelX = levelX - GetStringWidth(FONT_SMALL_NARROWER, sText_Lv, 0) - 1;
        nameRight = levelLabelX;
    }

    // print nickname, narrowing the font if it would run into the level
    GetMonNickname(mon, gStringVar1);
    PrintTextOnWindowToFitPx_WithFont(PSS_LABEL_WINDOW_PORTRAIT_INFO, gStringVar1, PORTRAIT_NAME_X, 1, 0, 1,
                                      PSS_DEFAULT_FONT, nameRight - PORTRAIT_NAME_X - 2);

    // See CreateGenderSprite function, not using gender symbols (text)
    // PrintGenderSymbol(mon, summary->species2);

    // print level only if no status condition, the status icon covers this spot
    if (statusAnim == 0)
    {
        PrintTextOnWindowWithFont(PSS_LABEL_WINDOW_PORTRAIT_INFO, sText_Lv, levelLabelX, 1, 0, 1, FONT_SMALL_NARROWER);
        PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_INFO, gStringVar2, levelX, 1, 0, 1);
    }
...
static void PrintEggStepsRemaining(void)
{
    ...
    u8 stringXPos = GetStringRightAlignXOffset(PSS_DEFAULT_FONT, gStringVar1, 80) + 40;
...
static void PrintEggInfo(void)
{
    ...
    PrintTextOnWindow(PSS_LABEL_WINDOW_PORTRAIT_INFO, gStringVar1, 20, 1, 0, 1);
...
static void PrintMonAbilityDescription(void)
{
    ...
    if (SWSH_SUMMARY_SHOW_DYNAMAX_LEVEL)
    {
        // Dynamax layout only leaves room for a single line
        PrintTextOnWindow(windowId, gAbilitiesInfo[ability].description, 0, 22, 0, 0);
        return;
    }

    // Y position per line count so the block stays centered under the Ability label
    static const u8 sDescriptionY[] = {28, 28, 21, 14};

    u8 desc[SWSH_MAX_ABILITY_DESCRIPTION_LENGTH];
    u32 lineCount;
    u8 fontId = SwSh_FormatTextByWidthLines(desc, 144, PSS_DEFAULT_FONT, gAbilitiesInfo[ability].description,
                                            GetFontAttribute(PSS_DEFAULT_FONT, FONTATTR_LETTER_SPACING), 3, &lineCount);

    if (lineCount >= ARRAY_COUNT(sDescriptionY))
        lineCount = ARRAY_COUNT(sDescriptionY) - 1;

    PrintTextOnWindowWithFont(windowId, desc, 0, sDescriptionY[lineCount], 0, 0, fontId);
}
...
static void PrintHeldItemInfo(void)
{
    ...
    fontId = SwSh_FormatTextByWidth(desc, 144, PSS_DEFAULT_FONT, description, GetFontAttribute(PSS_DEFAULT_FONT, FONTATTR_LETTER_SPACING));
...
static void PrintMoveNameAndPP(u8 slotIndex)
{
    ...
    u16 move = summary->moves[slotIndex];
...
static void PrintMoveDescription(u16 move)
{
    ...
    if (move != MOVE_NONE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
        {
            ...
            if (SWSH_SUMMARY_AUTO_FORMAT_MOVE_DESCRIPTIONS)
            {
                ...
                if (gMovesInfo[move].effect != EFFECT_PLACEHOLDER)
                    descFontId = SwSh_FormatTextByWidth(desc, 136, PSS_DEFAULT_FONT, gMovesInfo[move].description, GetFontAttribute(PSS_DEFAULT_FONT, FONTATTR_LETTER_SPACING));
                else
                    descFontId = SwSh_FormatTextByWidth(desc, 136, PSS_DEFAULT_FONT, gNotDoneYetDescription, GetFontAttribute(PSS_DEFAULT_FONT, FONTATTR_LETTER_SPACING));
        ...
        else if (sMonSummaryScreen->currPageIndex == PSS_PAGE_CONTEST_MOVES)
        {
            ...
            if (SWSH_SUMMARY_AUTO_FORMAT_MOVE_DESCRIPTIONS)
            {
                u8 descFontId = SwSh_FormatTextByWidth(desc, 136, PSS_DEFAULT_FONT, gContestEffects[GetMoveContestEffect(move)].description, GetFontAttribute(PSS_DEFAULT_FONT, FONTATTR_LETTER_SPACING));
                PrintTextOnWindowWithFont(windowId, desc, 0, 4, 1, 0, descFontId);
            }
            else
            {
...
static void PrintHMMovesCantBeForgotten(void)
{
    ...
    u8 msgFontId = SwSh_FormatTextByWidth(message, 136, PSS_DEFAULT_FONT, gText_HMMovesCantBeForgotten2, GetFontAttribute(PSS_DEFAULT_FONT, FONTATTR_LETTER_SPACING));
...
static void SetPokerusCuredSprite(void)
{
    ...
    gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_POKERUS_CURED]].invisible = !CheckPartyPokerus(mon, 0);
}
...
// Original SwSh_FormatTextByWidth function by Vexx on Ravepossum's branch
// Modified here to use PerformTextFormatting and try 1-font narrower if needed
static u8 SwSh_FormatTextByWidthLines(u8 *result, s32 maxWidth, u8 fontId, const u8 *str, s16 letterSpacing, u32 maxLines, u32 *outLineCount)
{
    bool32 lastLineFits;

    // Try formatting with progressively narrower fonts until it fits in maxLines or fewer
    while (TRUE)
    {
        lastLineFits = PerformTextFormatting(result, maxWidth, fontId, str, letterSpacing, outLineCount);

        // If we are within the line budget AND the last line fits, we're done
        if (*outLineCount <= maxLines && lastLineFits)
            break;
...
static u8 SwSh_FormatTextByWidth(u8 *result, s32 maxWidth, u8 fontId, const u8 *str, s16 letterSpacing)
{
    u32 lineCount;
    return SwSh_FormatTextByWidthLines(result, maxWidth, fontId, str, letterSpacing, 2, &lineCount);
}
...
static void CB2_PssChangePokemonNickname(void)
{
    ...
    DoNamingScreen(NAMING_SCREEN_NICKNAME, gStringVar2, GetMonData(mon, MON_DATA_SPECIES, NULL),
                   GetMonGender(mon), GetMonData(mon, MON_DATA_PERSONALITY, NULL),
