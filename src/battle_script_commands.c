...
static inline bool32 TrySetReflect(u32 battler)
{
    ...
    if (!(gSideStatuses[side] & SIDE_STATUS_REFLECT))
    {
        ...
        gSideTimers[side].reflectTimerTotal = gSideTimers[side].reflectTimer;
...
static inline bool32 TrySetLightScreen(u32 battler)
{
    ...
    if (!(gSideStatuses[side] & SIDE_STATUS_LIGHTSCREEN))
    {
        ...
        gSideTimers[side].lightscreenTimerTotal = gSideTimers[side].lightscreenTimer;
...
static void SetNonVolatileStatus(u32 effectBattler, enum MoveEffect effect, const u8 *battleScript, enum StatusTrigger trigger)
{
    ...
    case MOVE_EFFECT_SLEEP:
        if (B_SLEEP_TURNS_MAX_TWO)
            gBattleMons[effectBattler].status1 |= STATUS1_SLEEP_TURN(RandomChance(RNG_SLEEP_TURNS, 1, 3) ? 2 : 3);
...
void SetMoveEffect(u32 battler, u32 effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_HAIL:
    {
        ...
        if (TryChangeBattleWeather(gBattlerAttacker, weather, ABILITY_NONE) == WEATHER_FAILURE_SUCCESS)
        {
    ...
    case MOVE_EFFECT_AURORA_VEIL:
        if (!(gSideStatuses[GetBattlerSide(gBattlerAttacker)] & SIDE_STATUS_AURORA_VEIL))
        {
            ...
            gSideTimers[GetBattlerSide(gBattlerAttacker)].auroraVeilTimerTotal = gSideTimers[GetBattlerSide(gBattlerAttacker)].auroraVeilTimer;
...
static void Cmd_setfieldweather(void)
{
    ...
    if (TryChangeBattleWeather(gBattlerAttacker, battleWeatherId, ABILITY_NONE) != WEATHER_FAILURE_SUCCESS)
    {
        gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_MISSED;
...
void ApplyExperienceMultipliers(s32 *expAmount, u8 expGetterMonId, u8 faintedBattler)
{
    ...
    if (B_AFFECTION_MECHANICS == TRUE && GetMonAffectionHearts(&gPlayerParty[expGetterMonId]) >= AFFECTION_FOUR_HEARTS)
        *expAmount = (*expAmount * 4915) / 4096;
    if (CheckBagHasItem(ITEM_POKEVIAL, 1)) //is also for other exp boosting Powers if/when implemented
        *expAmount = (*expAmount * 150) / 100;
...
void BS_TryActivateLastKiss(void)
{
    NATIVE_ARGS();
     ...
     && !HasNoMonsToSwitch(battler, PARTY_SIZE, PARTY_SIZE))
    {
        ...
        gBattleScripting.battler = battler;
        BattleScriptPush(cmd->nextInstr);
        gBattlescriptCurrInstr = BattleScript_LastKissActivates;
    }
    else
    {
        gBattlescriptCurrInstr = cmd->nextInstr;
...
void BS_SetAuroraVeil(void)
{
    ...
    else
    {
        ...
        if (GetBattlerHoldEffect(gBattlerAttacker) == HOLD_EFFECT_LIGHT_CLAY)
            ...
            gSideTimers[GetBattlerSide(gBattlerAttacker)].auroraVeilTimer = 5;
        gSideTimers[side].auroraVeilTimerTotal = gSideTimers[side].auroraVeilTimer;
