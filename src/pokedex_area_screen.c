...
static bool8 MonListHasSpecies(const struct WildPokemonHeader *, u16, WildArea);
...
static bool8 MonListHasSpecies(const struct WildPokemonHeader *header, u16 species, WildArea area)
{
    u16 i, size;
    ...
    switch(area){
        case WILD_AREA_WATER:
            info = header->encounterTypes[gAreaTimeOfDay].waterMonsInfo;
            size = WATER_WILD_COUNT;
            break;
        case WILD_AREA_ROCKS:
            info = header->encounterTypes[gAreaTimeOfDay].rockSmashMonsInfo;
            size = ROCK_WILD_COUNT;
            break;
        case WILD_AREA_FISHING:
            info = header->encounterTypes[gAreaTimeOfDay].fishingMonsInfo;
            size = FISH_WILD_COUNT;
            break;
        case WILD_AREA_LAND:
        default:
            info = header->encounterTypes[gAreaTimeOfDay].landMonsInfo;
