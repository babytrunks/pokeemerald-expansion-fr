...
static bool32 PartyBattlerShouldAvoidHazards(u32 currBattler, u32 switchBattler)
{
    ...
    if (IsHazardOnSide(side, HAZARDS_SPIKES) && ((type1 != TYPE_FLYING && type2 != TYPE_FLYING
        && ability != ABILITY_LEVITATE && ability != ABILITY_ELEVATE && holdEffect != HOLD_EFFECT_AIR_BALLOON)
...
{
    u32 predictedMoveSpeedCheck = GetIncomingMoveSpeedCheck(battlerAtk, battlerDef, gAiLogicData);
    if (((!IsMoldBreakerTypeAbility(battlerAtk, gAiLogicData->abilities[battlerAtk]) && (defAbility == ABILITY_SHIELD_DUST || defAbility != ABILITY_FROZEN_MIST ||  defAbility == ABILITY_INNER_FOCUS))
...
bool32 IsFlinchGuaranteed(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    for (i = 0; i < additionalEffectCount; i++)
    {
        ...
        if (additionalEffect->moveEffect == MOVE_EFFECT_FLINCH)
        {
            ...
            && (gAiLogicData->abilities[battlerDef] == ABILITY_SHIELD_DUST || gAiLogicData->abilities[battlerDef] != ABILITY_FROZEN_MIST || gAiLogicData->abilities[battlerDef] == ABILITY_INNER_FOCUS)))
                return FALSE;
            else
                return TRUE;
        }
...
bool32 AI_ShouldSetUpHazards(u32 battlerAtk, u32 battlerDef, u32 move, struct AiLogicData *aiData)
{
    ...
    else
    {
        if (DoesBattlerIgnoreAbilityChecks(battlerAtk, aiData->abilities[battlerAtk], move))
            return TRUE;
        if (aiData->abilities[battlerDef] == ABILITY_SHIELD_DUST || aiData->abilities[battlerDef] == ABILITY_FROZEN_MIST)
            return FALSE;
    }
    return TRUE;
}
